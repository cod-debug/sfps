...
$fees = $conn->query("SELECT ef.*,s.name as sname,s.id_no,concat(c.course,' - ',c.level) as `class`, c.course as course FROM student_ef_list ef inner join student s on s.id = ef.student_id inner join courses c on c.id = ef.course_id  where ef.id = {$_GET['ef_id']}");

foreach($fees->fetch_array() as $k => $v){
	$$k= $v;
}
$crs = $conn->query("SELECT * FROM `course_list` WHERE `cl_id` = '$course'");
$course = $crs->fetch_assoc();
...
<div class="container-fluid">
	...
	<div class="flex">
		<div class="w-50">
			...
			<p>Course/Level: <b><?php echo $course["course_name"] ?></b></p>
		</div>
		<?php if($_GET['pid'] > 0): ?>
		<div class="w-50">
