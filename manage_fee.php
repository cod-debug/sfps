		...
		<div class="form-group">
			...
			<select name="course_id" id="course_id" class="custom-select input-sm select2">
				...
				<?php
					...
					while($row= $student->fetch_assoc()):
						$course_list = $conn->query("SELECT * FROM `course_list` WHERE `cl_id` = '$row[course]'");
						$cl = $course_list->fetch_assoc();
				?>
				<option value="<?php echo $row['id'] ?>" data-amount = "<?php echo $row['total_amount'] ?>" <?php echo isset($course_id) && $course_id == $row['id'] ? 'selected' : '' ?>><?php echo $cl['course_name'].' - '.$row['level'] ?></option>
				<?php endwhile; ?>
			</select>
		</div>
		 <div class="form-group">
