...
Class Action {
	...
	function student_login(){
		...
		$qry = $this->db->query("SELECT * FROM student where email = '".$email."' and password = '".md5($password)."' ");
		if($qry->num_rows > 0){
			foreach ($qry->fetch_array() as $key => $value) {
				if($key != 'passwors' && !is_numeric($key))
					$_SESSION['login_'.$key] = $value;
	...
	function save_online_payment(){
		...
		$save = $this->db->query("INSERT INTO online_payments set $data");
		if ($save) {
			return 1;
		} else {
			return 2;
	...
	function approve_online_payment(){
		extract($_POST);
		$data = "";
		foreach($_POST as $k => $v){
			if($k != 'op_id'){
		...
		$save = $this->db->query("UPDATE online_payments set $data WHERE `op_id`='$_POST[op_id]'");
		if ($save) {
			return 1;
		} else {
			return 2;
